...
{
    try {
        handleTimePassed(*static_cast<EventT<TimeoutInd> const&>(*e->clone()));
    } catch (std::bad_cast&) {
        try {
            handleDirectionChange(*static_cast<EventT<DirectionInd> const&>(*e->clone()));
        } catch (std::bad_cast&) {
            try {
                handleFoodPositionChange(*static_cast<EventT<FoodInd> const&>(*e->clone()));
            } catch (std::bad_cast&) {
                try {
                    handleNewFood(*static_cast<EventT<FoodResp> const&>(*e->clone()));
                } catch (std::bad_cast&) {
